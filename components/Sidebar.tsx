...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  const menuItems = [
    ...
    { id: 'blueprint', label: '系統架構', icon: 'fa-sitemap' },
  ];
  ...
  return (
    <div className="w-64 h-screen bg-slate-900 text-white flex flex-col fixed left-0 top-0 z-50">
      <div className="p-8 flex items-center gap-3 border-b border-slate-800">
        ...
        <div>
          <h1 className="font-bold text-xl leading-tight">千尋派車</h1>
          <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Chihiro Dispatch</p>
        </div>
      </div>
      ...
      <nav className="flex-1 mt-8 px-4 overflow-y-auto custom-scrollbar">
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl mb-2 transition-all duration-300 ${
              activeTab === item.id 
      ...
      <div className="p-6">
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-5 border border-slate-700">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">系統核心已啟動</span>
          </div>
